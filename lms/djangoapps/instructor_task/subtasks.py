...
def queue_subtasks_for_query(
    ...
    Arguments:
        ...
        `item_querysets` : query sets that define the "items" that should be passed to subtasks.
        `item_fields` : the fields that should be included in the dict that is returned.
            These are in addition to the 'pk' field.
        `item_order_by`: the field that the queryset should be ordered by.
        `items_per_query` : size of chunks to break the query operation into.
        `items_per_task` : maximum size of chunks to break each query chunk into for use by a subtask.
    ...
    queries = []
    Query = namedtuple('Query', ['item_queryset', 'num_items', 'num_subtasks'])
    for item_queryset in item_querysets:
        num_items = item_queryset.count()
        num_subtasks = _get_number_of_subtasks(num_items, items_per_query, items_per_task)
        queries.append(Query(item_queryset, num_items, num_subtasks))

    total_num_items = sum([query.num_items for query in queries])

    # Calculate the number of tasks that will be created, and create a list of ids for each task.
    total_num_subtasks = sum([query.num_subtasks for query in queries])
