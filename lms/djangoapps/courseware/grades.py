...
def _grade_section(section_descriptor, student, model_data_cache, module_creator):
    """
...
def grade(student, request, course, model_data_cache=None, keep_raw_scores=False):
    ...
    return _grade(student, course, grading_context, module_creator, model_data_cache, keep_raw_scores)
...
def grade_as_task(student, course, track_function, xqueue_callback_url_prefix):
    ...
    def module_creator(descriptor):
        ...
        module = get_module_for_descriptor_internal(
            student, descriptor, model_data_cache, course_id, track_function, xqueue_callback_url_prefix
        )
...
def _grade(student, course, grading_context, module_creator_fcn, model_data_cache, keep_raw_scores):
    """
    TODO:
    """
    raw_scores = []
    ...
    for section_format, sections in grading_context['graded_sections'].iteritems():
        ...
        for section in sections:
            ...
            for moduledescriptor in section['xmoduledescriptors']:
                ...
                if _get_student_module_from_cache(model_data_cache, student, moduledescriptor) is not None:
                    should_grade_section = True
                    break

            if should_grade_section:
                graded_total, scores = _grade_section(section_descriptor, student, model_data_cache, module_creator_fcn)
                if keep_raw_scores:
                    raw_scores += scores
            else:
...
def _get_student_module_from_cache(model_data_cache, student, problem_descriptor):
    """
    Find StudentModule object in model_data_cache.
    ...
    return model_data_cache.find(key)
...
def _get_score(user, problem_descriptor, module_creator, model_data_cache):
    ...
    student_module = _get_student_module_from_cache(model_data_cache, user, problem_descriptor)
