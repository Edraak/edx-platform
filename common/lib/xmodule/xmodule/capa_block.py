"""An XBlock implementing capa_problem.
"""
import cgi
import datetime
import hashlib
...
from .exceptions import NotFoundError, ProcessingError
# from xblock.core import Scope, String, Boolean, Dict, Integer, Float
from .fields import Timedelta, Date
from django.utils.timezone import UTC


from xblock.core import XBlock, Scope, String, Boolean, Dict, Integer, Float, ModelType
...
class LocationField(ModelType):
    ...
    def html_id(self):
        ...
        html id attributes
        """
        # s = "-".join(str(v) for v in list(self)
        #              if v is not None)
...
class CapaModule(XBlock):
    """
    An XBlock for capa_problem, potentially replacing capa_module

    """
# Part 1. Copying over all the fields into the block definition
    display_name = String(
...
# Part 2. All the methods
    def __init__(self, *args, **kwargs):
        """
        Accepts the same arguments as xmodule.x_module:XModule.__init__
        """
        XBlock.__init__(self, args[0], args[2])
    # hacks to make XBlock compatible
        # from xmodule/x_module.py
...
##########################################
    ...
    def student_view(self, context):
        ...
        css_str = {'scss': [resource_string(__name__, 'css/capa/display.scss')]}
        frag.add_css(unicode(css_str))

        # js_str = pkg_resources.resource_string(__name__,
        #                                        "static/js/src/thumbs.js")
        js_str = {'coffee': [resource_string(__name__, 'js/src/capa/display.coffee'),
        ...
        frag.initialize_js('Problem')
        return frag

    # problem_view = student_view

    def problem_view(self, context):
        frag = Fragment(unicode(self.get_problem_html().format(self=self)))
        return frag
